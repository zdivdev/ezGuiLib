...
class FxControl():
    def Initialize(self,h):
        ...
        if h.get('image'): self.SetIcon( h['image'], h.get('size'), h.get('icon_top') )
...
class FxText(FxControl):
    ...
    def AppendText(self,text): self.ctrl.appendText(text)
...
class FxProgressBar(FxControl):
    ...
    def SetValue(self,v):
        self.ctrl.setProgress(1.0*v/100)
...
def StartThread(handler,args=None):
    import threading
...
def AlertDialog(message, title=None, stage=None):
...
def YesNoDialog(message, title, stage=None):
    from javafx.scene.control import Alert
    from javafx.scene.control.Alert import AlertType
    from javafx.stage import Modality
...
class Window(Application):
    ...
    def SetFileDropHandler(self,handler): pass
